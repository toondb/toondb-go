...
|---------|----------|--------|
| Setup | `go get` only | Server + client |
...
**Note:** Requires `go get github.com/sochdb/sochdb-go[async]`
...
go build_native.go --libs

# Build for all platforms
go build_native.go --all

# Clean
go build_native.go --clean
```
...
# Specific test file
pytest tests/test_vector_search.go
...
```
sochdb/
├── __init__.go          # Public API exports
├── database.go          # Database, Transaction
├── namespace.go         # Namespace, Collection
├── vector.go            # VectorIndex, utilities
├── grpc_client.go       # SochDBClient (server mode)
├── ipc_client.go        # IpcClient (Unix sockets)
├── context.go           # ContextQueryBuilder
├── atomic.go            # AtomicMemoryWriter
├── recovery.go          # RecoveryManager
├── checkpoint.go        # CheckpointService
├── workflow.go          # WorkflowService
├── trace.go             # TraceStore
├── policy.go            # PolicyService
├── format.go            # WireFormat, ContextFormat
...
- [23_collections_embedded.go](examples/23_collections_embedded.go) - Document storage, JSON, transactions
- [22_namespaces.go](examples/22_namespaces.go) - Multi-tenant isolation with key prefixes
- [24_batch_operations.go](examples/24_batch_operations.go) - Atomic writes, rollback, conditional updates
- [25_temporal_graph_embedded.go](examples/25_temporal_graph_embedded.go) - Time-travel queries (NEW!)

**Server Mode (gRPC - Requires Server):**
- [21_temporal_graph.go](examples/21_temporal_graph.go) - Temporal graphs via gRPC
